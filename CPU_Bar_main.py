...
class Application(tk.Tk, ConfigureWidgets):                    # Наследуемся от класса создания основного окна
    def __init__(self):                                        # Перегружаем метод инит у класса
        tk.Tk.__init__(self)
        self.geometry("")                                      # Создание геометрии окна  400x400+50+1000 - ширина высота и отступы от угла
    ...
    def set_ui(self):                                          # Набор графических ВИДЖЕТОВ для интерфейса на главном окне
        exit_but = ttk.Button(self,                            # Создание кнопки с названием,  command- команда иполняемая по нажатию, это метод класса с названием app_exit
                              text='Exit',
        ...
        self.combo_win = ttk.Combobox(self.bar2,
                                      value=['hide',"don't hide",'min'],
                                      state='readonly',
                                      width=9)                 # Combobox-выпадающий список размещаем на фрейме бар2, передаем значения из выпадающего меню и только для чтения в список
        ...
        ttk.Button(self.bar2, text='>>>').pack(side=tk.LEFT)   # Создаем кнопку, размещаем на фрейме бар2, side - сторона

        self.bar1 = ttk.LabelFrame(self, text='Power')
        self.bar1.pack(fill=tk.BOTH)                           # Ввыравнивание по ширине

        self.test = ttk.Button(self.bar2, text='test').pack(side=tk.LEFT)

        self.bind_class('Tk', '<Enter>',self.enter_mouse)      # Позволяет сделать окно появляющимся bind - обрабатывает события наведения нажатия и прочее. При наведение на класс-Tk. Будут вызываться методы
        self.bind_class('Tk', '<Leave>',self.leave_mouse)      # Enter - при наведении мыши Leave - при покидании

        self.combo_win.bind('<<ComboboxSelected>>',
                            self.choise_combo)                 # При выборе пунктов используем метод сомбо чойз

    def make_bar_cpu_usage(self):
        ttk.Label(self.bar1,
                  text=f'physical_cores: {self.cpu.cpu_count}, logical cores:{self.cpu.cpu_count_logical}',             #lable  - метка(название, шапка) на фрейме Power
                  anchor=tk.CENTER).pack(fill=tk.X)            # anchor - якорь прикрепляем как метку к ередине экрана и устанавливаем по ширине

        self.list_label = []                                   # Имя, вставлен Label, который будет заполнен значением методом configure
        self.list_pbar = []                                    # Прогресс бар,расширен методом Progressbar,заполнено значением методом configure
    ...
    def leave_mouse(self,event):
        if self.combo_win.current() == 0:
           self.geometry(f'{self.winfo_width()}x28')            # Получаем текущее значение размеров окна и меняем его, если выбран пункт 1 - свернуть приложение
